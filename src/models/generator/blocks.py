...
class Noise(nn.Module):
    ...
    def forward(self, x):
        ...
        ----------
        x: tensor
            The tensor on which the transformation of injecting noise
...
class BasicSynthesisBlock(nn.Module):
    ...
    def forward(self, x, w):
        ...
        w: tensor
            The 'style' tensor. AdaIN transformations use it to transform
            the x tensor
...
class FirstSynthesisBlock(nn.Module):
    ...
    -------
    forward(w)
        Given a style 'w', it returns a tensor after transforming the constant
        using the basic block
    """
